...
export async function submitContact(req, res) {
  try {
    ...
    // SAVE FIRST
    const saved = await Contact.create(info);

    // Respond FAST
    res.json({ success:true, message:"Message received", id: saved._id });

    // Background jobs (never crash server)
    try {
      await sendEmail(process.env.ADMIN_EMAIL, "New Contact", JSON.stringify(info));
    } catch (e) {
      console.log("Email failed:", e.message);
    }

    try {
      await sendWhatsApp(`New Contact ${info.fullName} ${info.phone}`);
    } catch (e) {
      console.log("WhatsApp failed:", e.message);
    }

  } catch (err) {
    console.log("CONTACT API CRASH:", err.message);
